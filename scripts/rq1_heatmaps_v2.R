...
# ============================================================================ #
#   STEP 8B: Home-region overlay setup for FIGURE 2                            #
#   Tile fill stays unchanged; overlay shows in-state share within home region #
#   as a horizontal line starting at the left edge of the home-region tile     #
# ============================================================================ #
home_line_color_main   <- "white"
home_line_color_under  <- "black"
home_line_alpha_main   <- 1
home_line_alpha_under  <- 0.35
home_line_width_main   <- 1.15
home_line_width_under  <- 1.8
home_line_y_nudge      <- 0

build_home_overlay_lines <- function(
    df,
    x_var,
    y_var,
    prop_var,
  ...
  df %>%
    mutate(
      x_num    = match(as.character(.data[[x_var]]), x_levels),
      y_num    = match(as.character(.data[[y_var]]), y_levels),
      prop_val = pmax(pmin(.data[[prop_var]], 100), 0),
      line_w   = tile_width * prop_val / 100,
      x_left   = x_num - tile_width / 2,
      xstart   = x_left,
      xend     = x_left + line_w,
      y        = y_num + home_line_y_nudge
    ) %>%
    select(xstart, xend, y)
}

# Recruiting overlay numerator/denominator:
# numerator   = in-state visits within home EPS region
# denominator = all visits within home EPS region
recruit_home_overlay <- pubprivhs_univ_df %>%
  filter(univ_id != "all") %>%
  mutate(univ_id = as.character(univ_id)) %>%
  filter(!is.na(hs_eps_region), !is.na(univ_eps_region)) %>%
  filter(as.character(hs_eps_region) == as.character(univ_eps_region)) %>%
  group_by(univ_id, univ_abbrev, univ_eps_region, univ_state_code) %>%
  summarise(
    home_region_vis          = sum(num_visits, na.rm = TRUE),
    in_state_home_region_vis = sum(if_else(hs_state_code == univ_state_code, num_visits, 0), na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    prop_instate_within_home_region = if_else(
      home_region_vis > 0,
      100 * in_state_home_region_vis / home_region_vis,
      NA_real_
    ),
    hs_eps_region = factor(as.character(univ_eps_region), levels = region_order)
  ) %>%
  left_join(recruit_region_label_map, by = "univ_abbrev") %>%
  mutate(univ_label = factor(univ_label, levels = recruit_region_label_levels))

recruit_home_overlay_lines <- build_home_overlay_lines(
  df       = recruit_home_overlay,
  x_var    = "hs_eps_region",
  y_var    = "univ_label",
  prop_var = "prop_instate_within_home_region",
  x_levels = region_order,
  y_levels = recruit_region_label_levels,
  tile_width = 0.95
)

# Enrollment overlay numerator/denominator:
# numerator   = in-state freshmen within home EPS region
# denominator = all U.S. freshmen within home EPS region
enroll_home_overlay <- ipeds_migration_non_collapse_1617 %>%
  select(unitid, univ_abbrev, univ_state_code, univ_eps_region, all_of(state_cols)) %>%
  tidyr::pivot_longer(
    cols      = all_of(state_cols),
    names_to  = "state_col",
    values_to = "enrollment"
  ...
  filter(!is.na(hs_eps_region), hs_eps_region == as.character(univ_eps_region)) %>%
  group_by(unitid, univ_abbrev, univ_state_code, univ_eps_region) %>%
  summarise(
    home_region_enroll          = sum(enrollment, na.rm = TRUE),
    in_state_home_region_enroll = sum(if_else(hs_state_code == univ_state_code, enrollment, 0), na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    prop_instate_within_home_region = if_else(
      home_region_enroll > 0,
      100 * in_state_home_region_enroll / home_region_enroll,
      NA_real_
    ),
    hs_eps_region = factor(as.character(univ_eps_region), levels = region_order)
  ) %>%
...
enroll_home_overlay_lines <- build_home_overlay_lines(
  df       = enroll_home_overlay,
  x_var    = "hs_eps_region",
  y_var    = "enroll_label",
  prop_var = "prop_instate_within_home_region",
  x_levels = region_order,
  y_levels = enroll_label_levels,
  tile_width = 0.95
)
...
#   Tile fill = share in each EPS region                                       #
#   Overlay   = in-state share within home EPS region                          #
# ============================================================================ #
