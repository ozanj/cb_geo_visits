...
## [ AUTH ] < Ozan Jaquette >
## [ INIT ] < 8/8/2025
## [ DESC ] < exploratry descriptive statistics>
################################################################################
...
####### SOME EDA STARTS HERE, CREATED 8/8/2025
    pubprivhs_univ_df %>% 
...
##### SIMPLE DESCRIPTIVE STATS FOR CONNECTICUT COLLEGE
    pubprivhs_univ_df %>% glimpse()
    
    pubprivhs_univ_df %>% 
      filter(visit01==1, !is.na(hs_univ_market), univ_id == 128902) %>%
      count(hs_eps_region) %>% mutate(pct = n/sum(n)*100) %>% print(n=160) # most visits by CT College in new_england or middle_states
    
    
    pubprivhs_univ_df %>% filter(univ_id == 128902, hs_eps_region %in% c('new_england','middle_states')) %>% count() # number of high schools in these regions
    pubprivhs_univ_df %>% filter(univ_id == 128902, hs_eps_region %in% c('new_england','middle_states')) %>% count(hs_eps_region)
    pubprivhs_univ_df %>% filter(univ_id == 128902, hs_eps_region %in% c('new_england','middle_states')) %>% count(visit01)
    
    pubprivhs_univ_df %>% filter(univ_id == 128902, hs_eps_region %in% c('new_england','middle_states')) %>% count(hs_eps_codename) %>% print(n=400)
    pubprivhs_univ_df %>% filter(univ_id == 128902) %>% count(hs_eps_codename,visit01) %>% print(n=400)
    
    pubprivhs_univ_df %>% filter(univ_id == 128902, hs_eps_region %in% c('new_england','middle_states')) %>% group_by(hs_eps_codename) %>% summarize(
      n_schools = n(),
      n_visit = sum(visit01 ==1),
      n_no_visit = sum(visit01 ==0),
      pct_visit = n_visit/n_schools*100
    ) %>% print(n=100)
      
      count(visit01) %>% print(n=400)
