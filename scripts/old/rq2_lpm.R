...
mk_forms <- function(rhs_vec) {
  ...
  list(
    ...
    # (4) Univ × County FE only
    univ_county     = as.formula("visit01 ~ one | interaction(univ_id, hs_county_geoid)"),
    ...
    # (7) Covariates + Univ × EPS FE
    cov_univ_eps    = as.formula(paste0("visit01 ~ ", rhs_str, " | interaction(univ_id, hs_eps_codename)")),
    # (8) Covariates + Univ × County FE
    cov_univ_county = as.formula(paste0("visit01 ~ ", rhs_str, " | interaction(univ_id, hs_county_geoid)"))
  )
}
fit_models <- function(data, rhs_vec) {
  ...
  list(
    ...
    "(4) Covar + Univ FE"          = feols(f$cov_univ,        data = data, cluster = ~ hs_state_code + univ_id),
    "(5) Covar + Univ × State FE"  = feols(f$cov_univ_state,  data = data, cluster = ~ hs_state_code + univ_id),
    "(6) Covar + Univ × EPS FE"    = feols(f$cov_univ_eps,    data = data, cluster = ~ hs_state_code + univ_id),
