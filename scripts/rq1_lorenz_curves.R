...
build_scope <- function(df,
  ...
  if (scope == "all") {
    ...
    out <- df %>%
      ...
      dplyr::summarise(
        schools = max(.data[[school_var]], na.rm = TRUE),   # max() handles university-relative vars (instate/inregion/national)
...
plot_concentration <- function(cc, title = "Visits Concentration by Affluence",
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::geom_vline(xintercept = 0.5, color = "grey40") +
    ggplot2::geom_hline(yintercept = 0.5, color = "grey40") +
  ...
  build_one <- function(sv, vv, lab) {
    if (scope == "all") {
      ...
      out <- df %>%
        ...
        dplyr::summarise(
          series  = lab,
          schools = max(.data[[sv]], na.rm = TRUE),          # fixed: max() not first()
...
ses_concentration_multi <- function(scope_long, affluence_var = "mean_inc_house") {
  ...
  # Total visits per series (used for annotation in plot_concentration_multi)
  series_map <- attr(scope_long, "series_map")
  visit_totals <- series_map %>%
    dplyr::left_join(
      scope_long %>%
        dplyr::group_by(series) %>%
        dplyr::summarise(total_visits = sum(visits, na.rm = TRUE), .groups = "drop"),
      by = "series"
    ) %>%
    dplyr::select(series, total_visits)
...
plot_concentration_multi <- function(cc_multi, title = NULL, index_label = c("ATI", "ACI")) {
  ...
  # Upper-left annotation: visits and ATI per series
  if (!is.null(cc_multi$visit_totals)) {
    label_map <- c(
      "All schools"           = "All",
      "Public school visits"  = "Public",
      "Private school visits" = "Private"
    )
    vt <- cc_multi$visit_totals %>%
      dplyr::left_join(cc_multi$indices %>% dplyr::select(series, ATI), by = "series") %>%
      dplyr::mutate(label = dplyr::recode(series, !!!label_map))
    ann_text <- paste(
      sprintf("%s: %s visits, ATI = %.2f",
              vt$label,
              format(vt$total_visits, big.mark = ",", scientific = FALSE, trim = TRUE),
              vt$ATI),
      collapse = "\n"
    )
...
#   - show_x_axis removed: x-axis ticks/labels now always visible on every cell
#   - x-axis title always suppressed (use row/col labels for context instead)
#   - gridlines at every 0.1 on both axes; labels printed only at 0/.2/.4/.6/.8/1.
#   - gridline color: grey75 (darker than grey87, still well lighter than grey40 reference lines)
#   - axis tick label size: base_size * 0.9 for both x and y
#   - upper-left annotation size: base_size * 0.30
plot_concentration_cell <- function(cc_multi,
                                    show_y_axis = FALSE,
                                    col_title   = NULL,
                                    row_title   = NULL,
                                    ann_df      = NULL,   # data frame: label, n_visits, ati
                                    base_size   = 8) {
  ...
  breaks_01 <- seq(0, 1, by = 0.1)
  fmt_dot   <- function(x) { s <- sprintf("%.1f", x); ifelse(x < 1, sub("^0", "", s), s) }
  # Labels at 0, .2, .4, .6, .8, 1.0 only; gridlines at every .1
  label_fn  <- function(x) ifelse(round(x * 10) %% 2 == 0, fmt_dot(x), "")
  ...
  p <- ggplot2::ggplot(curves, ggplot2::aes(x = cs0, y = cv0, color = series)) +
    ...
    ggplot2::geom_vline(xintercept = 0.5, color = "grey40", linewidth = 0.3) +
    ggplot2::geom_hline(yintercept = 0.5, color = "grey40", linewidth = 0.3) +
    ggplot2::geom_line(linewidth = 0.5) +
    ggplot2::scale_x_continuous(
      limits = c(0, 1), breaks = breaks_01, labels = label_fn, expand = c(0, 0)
    ) +
    ggplot2::scale_y_continuous(
      limits = c(0, 1), breaks = breaks_01, labels = label_fn, expand = c(0, 0)
    ) +
    ggplot2::scale_color_discrete(name = NULL, labels = legend_labels) +
    ggplot2::labs(x = NULL, y = y_lab, title = col_title) +   # x title always suppressed
    ggplot2::coord_equal() +
    ggplot2::theme_minimal(base_size = base_size) +
    ggplot2::theme(
      panel.grid.minor  = ggplot2::element_blank(),
      panel.grid.major  = ggplot2::element_line(linewidth = 0.25, color = "grey75"),  # darker gridlines
      panel.background  = ggplot2::element_rect(fill = "grey97", color = NA),
  ...
  # Annotate upper-left with visits and ATI per series
  if (!is.null(ann_df)) {
    ann_label <- paste(
      sprintf("%s: %s | ATI = %.2f",
              ann_df$label,
              format(round(ann_df$n_visits), big.mark = ",", scientific = FALSE, trim = TRUE),
              ann_df$ati),
      collapse = "\n"
    )
    p <- p + ggplot2::annotate(
  ...
  if (!show_y_axis) p <- p + ggplot2::theme(
...
#
# Visit count annotation:
#   Each cell shows visits and ATI for all three series in the upper left.
#   Format: "All HS: N | ATI = X.XX" (one line per series)
#
# Note: show_x_axis removed — x-axis ticks/labels always shown on every cell.
plot_concentration_grid <- function(df,
  ...
  scope_cache <- vector("list", n_rows * n_cols)
  
  # Label mapping for cell annotations
  label_map <- c(
    "All schools"           = "All",
    "Public school visits"  = "Public",
    "Private school visits" = "Private"
  ...
  # ---- Phase 1: build all scope objects
  for (r in seq_len(n_rows)) {
    for (c in seq_len(n_cols)) {
      idx <- (r - 1) * n_cols + c
      scope_cache[[idx]] <- build_scope_multi(
        df,
        scope         = "group",
  ...
  # ---- Phase 2: build cell plots with annotations
  for (r in seq_len(n_rows)) {
    for (c in seq_len(n_cols)) {
      idx <- (r - 1) * n_cols + c
      cc  <- ses_concentration_multi(scope_cache[[idx]])
      
      ann_df <- cc$visit_totals %>%
        dplyr::left_join(cc$indices %>% dplyr::select(series, ATI), by = "series") %>%
        dplyr::mutate(
          label    = dplyr::recode(series, !!!label_map),
          n_visits = total_visits,
          ati      = ATI
        ) %>%
        dplyr::select(label, n_visits, ati)
...
# Upper-left annotation shows total visits and ATI per series.
...
# ATI values for the all-visits concentration curves (Graph 1)
ses_concentration_multi(scope_all)$indices
...
#   Upper-left annotation: visits and ATI per series
#   X-axis ticks/labels on every cell; gridlines at every .1
...
# Print ATI (affluence tilt index) for all 9 cells of the 3x3 concentration grid.
# Rows = geographic scope (in-state / in-region / national);
# Cols = institution type (private research / private lib arts / public research).
# Run this after plot_concentration_grid() to get the index values for write-up.

rows_def <- list(
  list(label = "In-state",         school_vars = c("n_sch_all_instate",  "n_sch_pub_instate",  "n_sch_priv_instate")),
  list(label = "In-region",        school_vars = c("n_sch_all_inregion", "n_sch_pub_inregion", "n_sch_priv_inregion")),
  list(label = "National",         school_vars = c("n_sch_all_national", "n_sch_pub_national", "n_sch_priv_national"))
)
cols_def <- list(
  list(label = "Private Research", group_vals = "private_national"),
...
for (r in seq_along(rows_def)) {
  for (c in seq_along(cols_def)) {
    scope_obj <- build_scope_multi(
      df_by_univ_eps,
      scope         = "group",
      group_vals    = cols_def[[c]]$group_vals,
      school_vars   = rows_def[[r]]$school_vars,
      series_labels = series_labels
    )
    cc <- ses_concentration_multi(scope_obj)
    cat(rows_def[[r]]$label, "|", cols_def[[c]]$label,
        "→ ATI:", round(cc$indices$ATI, 3), "\n")
  }
}