---
title: "Performing Social Closure:"
subtitle: 'How College Board Geomarkets Structure Recruiting Visits by Selective Colleges'
people:
  - name: Ozan Jaquette
    institute: UCLA
  - name: Karina Salazar
    institute: University of Arizona
  - name: Crystal Han
    institute: Stanford University
bibliography: ./../cb_geo/assets/bib/cb_geomarket_bib.bib
format:
  revealjs:
    revealjs-plugins: [multimodal]   # ← correct key
    theme: [default, ./assets/css/custom.scss]
    controls: true
    controls-layout: bottom-right
    controls-tutorial: true
    transition: slide
    background-transition: fade
    auto-stretch: false
    slide-level: 3
    menu:
      titleSelector: 'h1, h2, .slide:not(.hide) h3'
      useTextContentForMissingTitles: false
      hideMissingTitles: true
    template-partials:
      - ./assets/html/title-slide.html
include-in-header: ./assets/html/header.html
lightbox: false
---


# Introduction

```{r setup, include=FALSE}
library(tidyverse)
library(kableExtra)
mem.maxVSize(Inf)
knitr::opts_chunk$set(echo = FALSE, message = FALSE, warning = FALSE)
```


### __Motivation__

> The hierarchical structure of collegiate competition largely reflects the stratified social and economic dimensions of the communities from which colleges draw their students. Competition among colleges, as admissions officers have told us for so long, is in fact, a matter of keeping company with one's peers. — *The Structure of College Choice* [@RN4982]

**Social closure:** privileged status groups restrict access to resources that confer privilege

- Selective colleges are sites of social closure
- Scholarship largely focuses on **admissions** as the key mechanism
- But colleges do not passively receive applicants; they **recruit**
- If recruiting territories are structured by shared infrastructures, closure may begin **before admissions**

**Do College Board Geomarkets structure high school recruiting visits by selective colleges?**


### __Sociology of Education__
#### __How are students sorted into colleges?__

Explanations from *Annual Review of Sociology*

- <span style="color:#B3B3B3;">Households: status attainment, cultural capital</span>
- <span style="color:#B3B3B3;">Schools: "sorting machines" that assign categories (e.g., tracks)</span>
- Colleges
  - Selective admissions: which criteria ration scarce seats

::: {style="margin-top:0.6em;"}
:::

Colleges looking for patrons [@RN4517]

- In "enrollment economy," stability depends on enrollment [@RN2401]
  - Responsibility of "enrollment management" [@RN3519; @RN5045]
  - High school recruiting visits [@RN4758]

::: {style="margin-top:0.6em;"}
:::

Third-party intermediaries "occupy the interface between consumers and producers" [@RN5065, p. 1]

- Sociology studies consumer-facing intermediaries (e.g., rankings)
- But business-facing intermediaries sort students on behalf of colleges

### **Case**
#### **The Structure of College Choice [@RN4982]**

Project goal: "capture and quantify" (p. 11) knowledge of admissions officers

::: {style="margin-top:1.0em;"}
:::


Creates three geographic levels: region, state, community (Geomarket)

- Geomarkets carve states and metro areas into smaller geographic regions

::: {style="margin-top:1.0em;"}
:::

Create 4 student "market segments" based on SAT score-sending behavior

- local, in-state, regional, national
- local: send most scores to colleges within Geomarket
- regional: send most scores out-of-state but in-region (e.g., Midwest)

::: {style="margin-top:1.0em;"}
:::

Market Segment Model predicts how demand for a college varies by Geomarket

- Student market segment mostly a function of class
- Selective colleges primarily enroll "regional" and "national" students
- Most "regional" and "national" students are located in affluent Geomarkets
- Selective colleges should focus recruiting on affluent Geomarkets
  
### Geomarkets around Chicago-land {.hide}

::: columns
::: {.column width="50%"}

![](./assets/images/chicago_geomarkets.png)

<!-- <img src="./assets/images/nyc_geomarkets.png" data-width="60%"> -->
:::

::: {.column width="50%"}
**[University of Chicago admissions counselors, IL territories](https://collegeadmissions.uchicago.edu/contact)**
![](./assets/images/chicagoland_territories_u_chicago_redacted.png)
:::
:::


### __Theory__
#### __Market devices and classification situations__

__Market devices literature__ [@callon1998embeddedness; @RN5064]

- **Market devices**: tools that calculate/classify, then get incorporated in products
  - Geomarkets: defines recruiting territories
  - Market Segment Model: theory is abstract idea, until embedded in a device
    - Enrollment Planning Service (EPS) software

- **Agencement**: devices + org roles/goals produce menu of possible/likely actions  
  - Colleges commensurate Geomarkets based on market segments, SES, scores

- **Performativity**: Usage of devices produce behavior consistent with theory
  - Here: colleges use Geomarkets + related devices to produce recruiting behavior consistent with Market Segment Model

__Classification Situations__ [@RN4810; @RN5015]

- Ordinally ranked consumers matched to ordinally ranked products
- Market Segment Model: ordinally ranked student segments (local, in-state, regional, national) matched to ordinally ranked colleges


### __Theory__ {.hide}
#### __Research questions and expectations__

**RQ1**: Are visit patterns consistent with Market Segment Model recommendations?

- Expectation: For selective privates and most publics in sample, Market Segment Model recommends focusing on affluent Geomarkets in regions where many students likely to consider a college like yours

**RQ2**: Do Geomarkets (X) explain which high schools receive recruiting visits (Y)

- Geomarkets embedded in org + tech infrastructure that produces visits
  - Colleges assign admissions officers to Geomarkets ([LinkedIn profiles](#){data-modal-type="image" data-modal-url="./assets/images/strong_island_geomarkets_linkedin.png"})
  - Geomarkets + Market Segment Model basis for [Enrollment Planning Service](#){data-modal-type="image" data-modal-url="./assets/images/eps_landing_page.png"}
    - Identifies Geomarkets with high demand for peer/competitor colleges
  - Geomarkets integrated into [Slate/Oracle CRM software](#){data-modal-type="image" data-modal-url="./assets/images/slate_geomarket_shot.png"}

**RQ3**: Does Geomarket popularity (number of visits per school) mediate relationship between school composition (SES/race) and probability of a receiving a visit?

- Classification situations seek "good matches" between consumers and products
- A wealthy school in a wealthy Geomarket is a good match

# Methods

### __Data and sample__

Data collection [@RN4758]

- 2017 off-campus recruiting visits scraped from univ admissions websites

Data collection sample

- Public "research-extensive" universities from 2000 Carnegie (N=102)
- Private universities in top 100 of U.S. News *National Universities* (N=57)
- Private colleges in top 50 of *National Liberal Arts Colleges* (N=47)

Analysis sample: colleges that posted complete recruiting visits on website

- 16 public research, 14 private universities, 12 private liberal arts

Constructing Geomarkets

- Assign Census tracts to Geomarkets by spatial overlay of @RN4988


Secondary data

- IPEDS; CCD; Private School Survey; Niche *Best Public/Private High Schools*

# Results

## RQ1


### __RQ1: Do visit patterns match Market Segment Model?__ {.hide}
#### __Recruiting visits by market segment__

```{r fig-recruiting-heatmap-market-appendix, echo=FALSE, results="asis", message=FALSE, warning=FALSE}
pdf_in  <- "results/recruiting_heatmap_market_appendix.pdf"
png_out <- "results/recruiting_heatmap_market_appendix.png"
if (!file.exists(png_out) || file.info(png_out)$mtime < file.info(pdf_in)$mtime) {
  if (!requireNamespace("magick", quietly = TRUE)) {
    stop("Install 'magick': install.packages('magick')")
  }
  img <- magick::image_read_pdf(pdf_in, density = 220)
  magick::image_write(img, path = png_out, format = "png")
}
cat(sprintf(
  '<p style="text-align:center;"><img src="%s" style="width:70%%; height:auto; cursor:zoom-in;"></p>',
  png_out
))
```

### __RQ1: Do visit patterns match Market Segment Model?__ {.hide}
#### __Recruiting visits and freshman enrollment by market segment__

```{r fig-recruiting-enrollment-market-combined, echo=FALSE, results="asis", message=FALSE, warning=FALSE}
pdf_in  <- "results/recruiting_enrollment_market_combined.pdf"
png_out <- "results/recruiting_enrollment_market_combined.png"
if (!file.exists(png_out) || file.info(png_out)$mtime < file.info(pdf_in)$mtime) {
  if (!requireNamespace("magick", quietly = TRUE)) {
    stop("Install 'magick': install.packages('magick')")
  }
  img <- magick::image_read_pdf(pdf_in, density = 220)
  magick::image_write(img, path = png_out, format = "png")
}
cat(sprintf(
  '<p style="text-align:center;"><img src="%s" style="width:90%%; height:auto; cursor:zoom-in;"></p>',
  png_out
))
```

### __RQ1: Do visit patterns match Market Segment Model?__ {.hide}
#### __Recruiting visits by EPS region__

```{r fig-recruiting-region-only, echo=FALSE, results="asis", message=FALSE, warning=FALSE}
pdf_in  <- "results/recruiting_heatmap_region_appendix.pdf"
png_out <- "results/recruiting_heatmap_region_appendix.png"
if (!file.exists(png_out) || file.info(png_out)$mtime < file.info(pdf_in)$mtime) {
  if (!requireNamespace("magick", quietly = TRUE)) {
    stop("Install 'magick': install.packages('magick')")
  }
  img <- magick::image_read_pdf(pdf_in, density = 220)
  magick::image_write(img, path = png_out, format = "png")
}
cat(sprintf(
  '<p style="text-align:center; margin-top:0.2em;"><img src="%s" style="width:68%%; max-height:86vh; height:auto; cursor:zoom-in;"></p>',
  png_out
))
```

### __RQ1: Do visit patterns match Market Segment Model?__ {.hide}
#### __Recruiting visits and freshman enrollment by EPS region__

```{r fig-recruiting-enrollment-region-combined, echo=FALSE, results="asis", message=FALSE, warning=FALSE}
pdf_in  <- "results/recruiting_enrollment_region_combined.pdf"
png_out <- "results/recruiting_enrollment_region_combined.png"
if (!file.exists(png_out) || file.info(png_out)$mtime < file.info(pdf_in)$mtime) {
  if (!requireNamespace("magick", quietly = TRUE)) {
    stop("Install 'magick': install.packages('magick')")
  }
  img <- magick::image_read_pdf(pdf_in, density = 220)
  magick::image_write(img, path = png_out, format = "png")
}
cat(sprintf(
  '<p style="text-align:center;"><img src="%s" style="width:90%%; height:auto; cursor:zoom-in;"></p>',
  png_out
))
```

### __RQ1: Do visit patterns match Market Segment Model?__ {.hide}
#### __Top 30 Geomarkets ranked by visits per school (public schools)__
```{r top30-pub, echo=FALSE, message=FALSE, warning=FALSE}
readRDS("results/top30_geo_pub_sch.RDS") %>%
  kableExtra::kbl(
    digits  = 1,
    caption = NULL,
    align   = c("r", "l", rep("r", ncol(.) - 2))
  ) %>%
  kableExtra::kable_classic(full_width = TRUE) %>%
  kableExtra::kable_styling(font_size = 16) %>%
  kableExtra::scroll_box(height = "82vh", width = "100%")
```

### __RQ1: Do visit patterns match Market Segment Model?__ {.hide}
#### __Top 30 Geomarkets ranked by visits per school (private schools)__
```{r top30-priv, echo=FALSE, message=FALSE, warning=FALSE}
readRDS("results/top30_geo_priv_sch.RDS") %>%
  kableExtra::kbl(
    digits  = 1,
    caption = NULL,
    align   = c("r", "l", rep("r", ncol(.) - 2))
  ) %>%
  kableExtra::kable_classic(full_width = TRUE) %>%
  kableExtra::kable_styling(font_size = 16) %>%
  kableExtra::scroll_box(height = "82vh", width = "100%")
```

### __RQ1: Do visit patterns match Market Segment Model?__ {.hide}
#### __Top 30 Geomarkets ranked by visits per school (public schools, national market segment)__
```{r top30-pub-national, echo=FALSE, message=FALSE, warning=FALSE}
readRDS("results/top30_geo_pub_national_sch.RDS") %>%
  kableExtra::kbl(
    digits  = 1,
    caption = NULL,
    align   = c("r", "l", rep("r", ncol(.) - 2))
  ) %>%
  kableExtra::kable_classic(full_width = TRUE) %>%
  kableExtra::kable_styling(font_size = 16) %>%
  kableExtra::scroll_box(height = "82vh", width = "100%")
```

### __RQ1: Do visit patterns match Market Segment Model?__ {.hide}
#### __Top 30 Geomarkets ranked by visits per school (private schools, national market segment)__
```{r top30-priv-national, echo=FALSE, message=FALSE, warning=FALSE}
readRDS("results/top30_geo_priv_national_sch.RDS") %>%
  kableExtra::kbl(
    digits  = 1,
    caption = NULL,
    align   = c("r", "l", rep("r", ncol(.) - 2))
  ) %>%
  kableExtra::kable_classic(full_width = TRUE) %>%
  kableExtra::kable_styling(font_size = 16) %>%
  kableExtra::scroll_box(height = "82vh", width = "100%")
```

### __RQ1: Do visit patterns match Market Segment Model?__ {.hide}
#### __Concentration of recruiting visits by Geomarket affluence__

```{r fig-concentration, echo=FALSE, results="asis", message=FALSE, warning=FALSE}
pdf_in  <- "results/concentration_all_visits.pdf"
png_out <- "results/concentration_all_visits.png"

if (!file.exists(png_out) || file.info(png_out)$mtime < file.info(pdf_in)$mtime) {
  if (!requireNamespace("magick", quietly = TRUE)) {
    stop("Install 'magick': install.packages('magick')")
  }
  img <- magick::image_read_pdf(pdf_in, density = 220)
  magick::image_write(img, path = png_out, format = "png")
}

cat(sprintf(
  '<p style="text-align:center; margin:0; padding:0;"><img src="%s" style="height:85vh; width:auto; cursor:zoom-in;"></p>',
  png_out
))
```

### __RQ1: Do visit patterns match Market Segment Model?__ {.hide}
#### __Concentration of recruiting visits by Geomarket affluence, by college type and market segment__

```{r fig-concentration-3x2, echo=FALSE, results="asis", message=FALSE, warning=FALSE}
pdf_in  <- "results/concentration_3x3.pdf"
png_out <- "results/concentration_3x3.png"

if (!file.exists(png_out) || file.info(png_out)$mtime < file.info(pdf_in)$mtime) {
  if (!requireNamespace("magick", quietly = TRUE)) {
    stop("Install 'magick': install.packages('magick')")
  }
  img <- magick::image_read_pdf(pdf_in, density = 220)
  magick::image_write(img, path = png_out, format = "png")
}

cat(sprintf(
  '<p style="text-align:center; margin:0; padding:0;"><img src="%s" style="height:85vh; width:auto; cursor:zoom-in;"></p>',
  png_out
))
```



## RQ2

### __RQ2: Do Geomarkets explain which schools get visits?__ {.hide}
#### __Fixed effects linear probability models of visits to public schools__

**Single-college model of visits to school $i$**

$$
Y_i = \alpha + X_i \beta + \gamma_{g(i)} + \varepsilon_i
$$

- $Y_i = 1$ if high school $i$ received $\geq 1$ recruiting visit.
- $X_i$: school- and neighborhood-level covariates
  - **School:** g12 enrollment, race/ethnicity, Niche quality grade, free/reduced lunch, math \& reading proficiency, 0/1 magnet, distance school $i$ to college $j$
  - **ZIP code:** household income, BA+ attainment, poverty, race/ethnicity
- $\gamma_{g(i)}$: fixed effect for geographic unit (e.g., Geomarket) containing school $i$

**School-college pair model, visits to school $i$ by college $j$**

$$
Y_{ij} = \alpha + X_i \beta + \gamma_{j \times g(i)} + \varepsilon_{ij}
$$

- $\gamma_{j \times g(i)}$: college-specific geographic fixed effects
  - Equal to dummy vars for each combo of college $j$ and geographic unit $g(i)$



```{r rq2-table-functions, echo=FALSE, include=FALSE}

make_rq2_fit_table <- function(tab_df) {
  
  fit_rows_num <- c(
    "N schools",
    "N (schools X colleges)",
    "Parameters (incl. FE)",
    "Residual df",
    "R²",
    "Adj. R²",
    "Within R²",
    "RMSE"
  )
  
  fmt_int_rows <- c(
    "N schools",
    "N (schools X colleges)",
    "Parameters (incl. FE)",
    "Residual df"
  )
  
  fmt_dec_rows <- c("R²", "Adj. R²", "Within R²", "RMSE")
  
  # highlight colors
  eps_col_bg       <- "#80D8FF"
  adjr2_row_bg     <- "#FFF4CC"
  intersection_bg  <- "#AEE8F2"
  
  # -----------------------------
  # Build numeric blocks first
  # -----------------------------
  tab_fe <- tab_df %>%
    filter(term %in% fit_rows_num) %>%
    select(term, 2:5) %>%
    setNames(c("term", "c1", "c2", "c3", "c4"))
  
  tab_cov <- tab_df %>%
    filter(term %in% fit_rows_num) %>%
    select(term, 6:9) %>%
    setNames(c("term", "c1", "c2", "c3", "c4"))
  
  # -----------------------------
  # Format numeric rows first
  # -----------------------------
  fmt_block <- function(df) {
    df %>%
      mutate(
        across(
          c(c1, c2, c3, c4),
          ~ dplyr::case_when(
            term %in% fmt_int_rows ~
              format(round(as.numeric(.), 0), big.mark = ",", scientific = FALSE, trim = TRUE),
            term %in% fmt_dec_rows ~
              sprintf("%.3f", as.numeric(.)),
            TRUE ~ as.character(.)
          )
        )
      ) %>%
      mutate(across(c(c1, c2, c3, c4), as.character))
  }
  
  tab_fe  <- fmt_block(tab_fe)
  tab_cov <- fmt_block(tab_cov)
  
  # -----------------------------
  # Add section rows LAST
  # -----------------------------
  tab_one <- bind_rows(
    tibble(
      term = "Without covariates",
      c1 = "\u200B(1)",
      c2 = "\u200B(2)",
      c3 = "\u200B(3)",
      c4 = "\u200B(4)"
    ),
    tab_fe,
    tibble(term = "", c1 = "", c2 = "", c3 = "", c4 = ""),
    tibble(term = "", c1 = "", c2 = "", c3 = "", c4 = ""),
    tibble(
      term = "With covariates",
      c1 = "\u200B(5)",
      c2 = "\u200B(6)",
      c3 = "\u200B(7)",
      c4 = "\u200B(8)"
    ),
    tab_cov
  )
  
  # -----------------------------
  # Add continuous borders across columns 2:5
  # Row 1  = model numbers for FE-only block
  # Row 9  = FE-only RMSE row
  # Row 12 = model numbers for covariate block
  # -----------------------------
  for (r in c(1, 9, 12)) {
    tab_one[r, c("c1", "c2", "c3", "c4")] <- lapply(
      tab_one[r, c("c1", "c2", "c3", "c4")],
      function(x) kableExtra::cell_spec(
        x,
        format = "html",
        extra_css = paste(
          "display:block;",
          "width:calc(100% + 28px);",
          "margin-left:-14px;",
          "margin-right:-14px;",
          "box-sizing:border-box;",
          "border-bottom:1px solid black;",
          "padding-left:14px;",
          "padding-right:14px;",
          "padding-bottom:2px;"
        )
      )
    )
  }
  
  # -----------------------------
  # Highlight intersection cells:
  # Adj. R² row x Univ × EPS FE column
  # Row 7  = Adj. R² in FE-only block
  # Row 18 = Adj. R² in covariate block
  # -----------------------------
  for (r in c(7, 18)) {
    tab_one[r, "c4"] <- lapply(
      tab_one[r, "c4"],
      function(x) kableExtra::cell_spec(
        x,
        format = "html",
        extra_css = paste(
          "display:block;",
          "width:calc(100% + 28px);",
          "margin-left:-14px;",
          "margin-right:-14px;",
          "margin-top:-8px;",
          "margin-bottom:-8px;",
          "box-sizing:border-box;",
          "padding:8px 14px;",
          paste0("background:", intersection_bg, ";")
        )
      )
    )
  }
  
  tab_one <- tab_one %>%
    rename(" " = term)
  
  tbl <- tab_one %>%
    kbl(
      format = "html",
      caption = NULL,
      escape = FALSE,
      col.names = c(
        " ",
        "Univ FE",
        "Univ × State FE",
        "Univ × County FE",
        "Univ × EPS FE"
      ),
      align = c("l", "r", "r", "r", "r"),
      table.attr = "style='margin-left:0; margin-right:auto; width:auto;'"
    ) %>%
    kable_styling(
      font_size = 14,
      full_width = FALSE,
      bootstrap_options = c("striped", "condensed"),
      position = "left"
    ) %>%
    row_spec(0, extra_css = "font-size: 1.15em;") %>%
    column_spec(1, width = "15em") %>%
    column_spec(
      2:5,
      width = "7em",
      extra_css = "padding-left: 14px; padding-right: 14px;"
    ) %>%
    row_spec(1, bold = TRUE) %>%
    row_spec(7, background = adjr2_row_bg) %>%
    row_spec(12, bold = TRUE) %>%
    row_spec(18, background = adjr2_row_bg) %>%
    row_spec(20, extra_css = "border-bottom: 2px solid black;") %>%
    column_spec(
      5,
      background = eps_col_bg,
      extra_css = "padding-left: 14px; padding-right: 14px;"
    )
  
  cat("\n\n<div style='display:block; width:max-content; margin-left:0; margin-right:auto;'>\n")
  print(tbl)
  cat("\n</div>\n\n")
  cat("<div style='font-size:0.5em; margin-top:8px; margin-left:0; text-align:left;'>Parameters include absorbed fixed effects.</div>\n\n")
}
```

### __RQ2: Do Geomarkets explain which schools get visits?__ {.hide}
#### __Model fit statistics, fixed effects linear probability models of visits to public and private schools__

<div style="height: 14px;"></div>

```{r rq2-model-fit-all, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

tab_df_all_ij <- readRDS("results/reg_tab_all_ij.RDS")
make_rq2_fit_table(tab_df_all_ij)

```

### __RQ2: Do Geomarkets explain which schools get visits?__ {.hide}
#### __Model fit statistics, Fixed effects linear probability models of visits to public schools__

<div style="height: 14px;"></div>

```{r rq2-model-fit-pub, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

tab_df_pub_ij <- readRDS("results/tab_pub_ij.RDS")
make_rq2_fit_table(tab_df_pub_ij)

```

### __RQ2: Do Geomarkets explain which schools get visits?__ {.hide}
#### __Model fit statistics, Fixed effects linear probability models of visits to private schools__

<div style="height: 14px;"></div>

```{r rq2-model-fit-priv, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

tab_df_priv_ij <- readRDS("results/tab_priv_ij.RDS")
make_rq2_fit_table(tab_df_priv_ij)

```


```{r rq2-border-table-functions, echo=FALSE, include=FALSE}

make_rq2_border_fit_table <- function(tab_df_nocov, tab_df_cov) {
  
  fit_rows_num <- c(
    "N schools",
    "N (schools X colleges)",
    "R²",
    "Adj. R²",
    "Within R²"
  )
  
  fmt_int_rows <- c(
    "N schools",
    "N (schools X colleges)"
  )
  
  fmt_dec_rows <- c("R²", "Adj. R²", "Within R²")
  
  adjr2_row_bg <- "#FFF4CC"
  
  fmt_block <- function(df) {
    df %>%
      dplyr::mutate(
        dplyr::across(
          c(c1, c2, c3, c4, c5, c6, c7, c8),
          ~ dplyr::case_when(
            term %in% fmt_int_rows ~
              format(round(as.numeric(.), 0), big.mark = ",", scientific = FALSE, trim = TRUE),
            term %in% fmt_dec_rows ~
              sprintf("%.3f", as.numeric(.)),
            TRUE ~ as.character(.)
          )
        )
      ) %>%
      dplyr::mutate(
        dplyr::across(c(c1, c2, c3, c4, c5, c6, c7, c8), as.character)
      )
  }
  
  solid_rule_cells <- function(x) {
    kableExtra::cell_spec(
      x,
      format = "html",
      extra_css = paste(
        "display:block;",
        "width:calc(100% + 16px);",
        "margin-left:-8px;",
        "margin-right:-8px;",
        "box-sizing:border-box;",
        "border-bottom:1px solid black;",
        "padding-left:8px;",
        "padding-right:8px;",
        "padding-bottom:2px;"
      )
    )
  }
  
  tab_fe <- tab_df_nocov %>%
    dplyr::filter(term %in% fit_rows_num) %>%
    dplyr::select(term, 2:9) %>%
    stats::setNames(c("term", "c1", "c2", "c3", "c4", "c5", "c6", "c7", "c8")) %>%
    fmt_block()
  
  tab_cov <- tab_df_cov %>%
    dplyr::filter(term %in% fit_rows_num) %>%
    dplyr::select(term, 2:9) %>%
    stats::setNames(c("term", "c1", "c2", "c3", "c4", "c5", "c6", "c7", "c8")) %>%
    fmt_block()
  
  tab_one <- dplyr::bind_rows(
    tibble::tibble(
      term = "Fixed effects",
      c1 = "State", c2 = "EPS",
      c3 = "State", c4 = "EPS",
      c5 = "State", c6 = "EPS",
      c7 = "State", c8 = "EPS"
    ),
    tibble::tibble(
      term = "Without covariates",
      c1 = "\u200B(1)", c2 = "\u200B(2)", c3 = "\u200B(3)", c4 = "\u200B(4)",
      c5 = "\u200B(5)", c6 = "\u200B(6)", c7 = "\u200B(7)", c8 = "\u200B(8)"
    ),
    tab_fe,
    tibble::tibble(
      term = "",
      c1 = "", c2 = "", c3 = "", c4 = "", c5 = "", c6 = "", c7 = "", c8 = ""
    ),
    tibble::tibble(
      term = "With covariates",
      c1 = "\u200B(9)",  c2 = "\u200B(10)", c3 = "\u200B(11)", c4 = "\u200B(12)",
      c5 = "\u200B(13)", c6 = "\u200B(14)", c7 = "\u200B(15)", c8 = "\u200B(16)"
    ),
    tab_cov
  )
  
  tab_one[1, c("c1","c2","c3","c4","c5","c6","c7","c8")] <- lapply(
    tab_one[1, c("c1","c2","c3","c4","c5","c6","c7","c8")],
    solid_rule_cells
  )
  
  tab_one[2, c("c1","c2","c3","c4","c5","c6","c7","c8")] <- lapply(
    tab_one[2, c("c1","c2","c3","c4","c5","c6","c7","c8")],
    solid_rule_cells
  )
  
  tab_one[7, c("c1","c2","c3","c4","c5","c6","c7","c8")] <- lapply(
    tab_one[7, c("c1","c2","c3","c4","c5","c6","c7","c8")],
    solid_rule_cells
  )
  
  tab_one[9, c("c1","c2","c3","c4","c5","c6","c7","c8")] <- lapply(
    tab_one[9, c("c1","c2","c3","c4","c5","c6","c7","c8")],
    solid_rule_cells
  )
  
  tab_one <- tab_one %>%
    dplyr::rename(" " = term)
  
  tbl <- tab_one %>%
    kableExtra::kbl(
      format = "html",
      caption = NULL,
      escape = FALSE,
      col.names = c(" ", rep("", 8)),
      align = c("l", rep("r", 8)),
      table.attr = "style='margin-left:0; margin-right:auto; width:auto; border-collapse:collapse;'"
    ) %>%
    kableExtra::add_header_above(
      c(" " = 1,
        "All schools" = 2,
        "≤ 2 miles" = 2,
        "≤ 1 mile" = 2,
        "≤ 0.5 miles" = 2),
      bold = TRUE,
      align = "c",
      line = FALSE
    ) %>%
    kableExtra::kable_styling(
      font_size = 16,
      full_width = FALSE,
      bootstrap_options = c("striped", "condensed"),
      position = "left"
    ) %>%
    kableExtra::row_spec(0, extra_css = "font-size: 1.35em; border-top: none; border-bottom: none;") %>%
    kableExtra::row_spec(1, bold = TRUE) %>%
    kableExtra::row_spec(2, bold = TRUE) %>%
    kableExtra::row_spec(6, background = adjr2_row_bg) %>%
    kableExtra::row_spec(8, extra_css = "height: 14px;") %>%
    kableExtra::row_spec(9, bold = TRUE) %>%
    kableExtra::row_spec(13, background = adjr2_row_bg) %>%
    kableExtra::row_spec(14, extra_css = "border-bottom: 1px solid black;") %>%
    kableExtra::column_spec(1, width = "13em") %>%
    kableExtra::column_spec(
      2:9,
      width = "4.6em",
      extra_css = "padding-left: 8px; padding-right: 8px; white-space: nowrap;"
    )
  
  cat("\n\n<div style='display:block; width:max-content; margin-left:0; margin-right:auto;'>\n")
  print(tbl)
  cat("\n</div>\n\n")
  cat("<div style='font-size:0.5em; margin-top:8px; margin-left:0; text-align:left;'>Parameters include absorbed fixed effects.</div>\n\n")
}
```

### __RQ2: Do Geomarkets explain which schools get visits?__ {.hide}
#### __Model fit, fixed effects LPMs of visits to public and private schools near Geomarket boundaries__


<div style="height: 14px;"></div>

```{r rq2-model-fit-all-border-compare, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

tab_df_all_bc_nocov <- readRDS("results/tab_all_border_compare_nocov_ij.RDS")
tab_df_all_bc_cov   <- readRDS("results/tab_all_border_compare_cov_ij.RDS")

make_rq2_border_fit_table(tab_df_all_bc_nocov, tab_df_all_bc_cov)

```

### __RQ2: Do Geomarkets explain which schools get visits?__ {.hide}
#### __Model fit, fixed effects LPMs of visits to public schools near Geomarket boundaries__

<div style="height: 14px;"></div>

```{r rq2-model-fit-pub-border-compare, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

tab_df_pub_bc_nocov <- readRDS("results/tab_pub_border_compare_nocov_ij.RDS")
tab_df_pub_bc_cov   <- readRDS("results/tab_pub_border_compare_cov_ij.RDS")

make_rq2_border_fit_table(tab_df_pub_bc_nocov, tab_df_pub_bc_cov)

```



## RQ3

```{r rq3-table-functions, echo=FALSE, include=FALSE}

make_rq3_table_df <- function(model_bundle, decile_cutoffs) {
  
  mod_base <- model_bundle$baseline_model
  mod_int  <- model_bundle$interaction_model
  
  fit_base <- model_bundle$fit_meta$baseline
  fit_int  <- model_bundle$fit_meta$interaction
  
  samp_base <- model_bundle$sample_meta$baseline
  samp_int  <- model_bundle$sample_meta$interaction
  
  spec_meta <- model_bundle$spec_meta
  
  pop_var <- spec_meta$popularity_var
  int_var <- spec_meta$interaction_var
  ref_lvl <- spec_meta$interaction_ref_level
  
  pop_row_label <- dplyr::case_when(
    spec_meta$pop_rate_type == "per_sch"  ~ "Visits per&nbsp;school",
    spec_meta$pop_rate_type == "per_g12k" ~ "Visits per 1k g12",
    TRUE ~ "Geomarket popularity measure"
  )
  
  decile_block_label <- dplyr::case_when(
    spec_meta$interaction_var == "hs_pct_free_reduced_lunch_decile" ~ "<span style='white-space:nowrap;'>Free/reduced lunch decile</span>",
    spec_meta$interaction_var == "hs_zip_inc_house_mean_decile"     ~ "<span style='white-space:nowrap;'>ZIP household income decile</span>",
    spec_meta$interaction_var == "hs_pct_bl_hisp_nat_decile"        ~ "<span style='white-space:nowrap;'>Pct Black/Brown/Native decile</span>",
    TRUE ~ "<span style='white-space:nowrap;'>Decile main effects</span>"
  )
  
  ct_base <- as.data.frame(fixest::coeftable(mod_base), stringsAsFactors = FALSE) %>%
    tibble::rownames_to_column("term") %>%
    tibble::as_tibble() %>%
    dplyr::transmute(
      term = term,
      coef_base = Estimate,
      se_base   = `Std. Error`,
      p_base    = `Pr(>|t|)`
    )
  
  ct_int <- as.data.frame(fixest::coeftable(mod_int), stringsAsFactors = FALSE) %>%
    tibble::rownames_to_column("term") %>%
    tibble::as_tibble() %>%
    dplyr::transmute(
      term = term,
      coef_int = Estimate,
      se_int   = `Std. Error`,
      p_int    = `Pr(>|t|)`
    )
  
  all_levels   <- paste0("D", 1:10)
  shown_levels <- setdiff(all_levels, ref_lvl)
  
  main_decile_terms <- paste0(int_var, shown_levels)
  interact_terms    <- paste0(int_var, shown_levels, ":", pop_var)
  
  target_terms <- c(
    pop_var,
    main_decile_terms,
    interact_terms
  )
  
  format_deciles <- Vectorize(function(x) {
    decile_num <- as.integer(stringr::str_replace(x, paste0(int_var, 'D'), ''))
    cutoffs <- decile_cutoffs[[str_replace(int_var, '_decile', '')]]
    if (is.na(decile_num)) return('')
    
    paste0('D', decile_num, ' (', round(cutoffs[[decile_num]]), '-', round(cutoffs[[decile_num + 1]]), '%)')
  })
  
  pretty_label <- function(x) {
    dplyr::case_when(
      x == pop_var ~ pop_row_label,
      x %in% main_decile_terms ~ format_deciles(x),
      x %in% interact_terms ~ paste0(
        "<span style='white-space:nowrap;'>Popularity × ",
        stringr::str_replace(
          stringr::str_replace(x, paste0(":", pop_var), ""),
          int_var,
          ""
        ),
        "</span>"
      ),
      TRUE ~ x
    )
  }
  
  structure_df <- tibble::tibble(term = target_terms) %>%
    dplyr::mutate(
      block = dplyr::case_when(
        term == pop_var ~ "Popularity",
        term %in% main_decile_terms ~ "Decile main effects",
        term %in% interact_terms ~ "Popularity × decile",
        TRUE ~ "Other"
      ),
      sort_key = dplyr::case_when(
        term == pop_var ~ 1,
        term %in% main_decile_terms ~ match(term, main_decile_terms) + 1,
        term %in% interact_terms ~ match(term, interact_terms) + 100,
        TRUE ~ 999
      ),
      term_label = pretty_label(term)
    )
  
  coef_df <- structure_df %>%
    dplyr::left_join(ct_base, by = "term") %>%
    dplyr::left_join(ct_int,  by = "term") %>%
    dplyr::arrange(sort_key) %>%
    dplyr::select(term_label, coef_base, se_base, p_base, coef_int, se_int, p_int, block)
  
  pop_header <- tibble::tibble(
    term_label = "<span style='white-space:nowrap;'>Geomarket popularity</span>",
    coef_base = NA_real_,
    se_base = NA_real_,
    p_base = NA_real_,
    coef_int = NA_real_,
    se_int = NA_real_,
    p_int = NA_real_,
    block = "header"
  )
  
  main_header <- tibble::tibble(
    term_label = decile_block_label,
    coef_base = NA_real_,
    se_base = NA_real_,
    p_base = NA_real_,
    coef_int = NA_real_,
    se_int = NA_real_,
    p_int = NA_real_,
    block = "header"
  )
  
  interact_header <- tibble::tibble(
    term_label = "<span style='white-space:nowrap;'>Popularity × Decile</span>",
    coef_base = NA_real_,
    se_base = NA_real_,
    p_base = NA_real_,
    coef_int = NA_real_,
    se_int = NA_real_,
    p_int = NA_real_,
    block = "header"
  )
  
  tab_coef <- dplyr::bind_rows(
    pop_header,
    coef_df %>% dplyr::filter(block == "Popularity"),
    main_header,
    coef_df %>% dplyr::filter(block == "Decile main effects"),
    interact_header,
    coef_df %>% dplyr::filter(block == "Popularity × decile")
  ) %>%
    dplyr::mutate(
      term_label = dplyr::case_when(
        block == "header" ~ term_label,
        TRUE ~ paste0("&nbsp;&nbsp;&nbsp;", term_label)
      )
    )
  
  fit_df <- tibble::tibble(
    term_label = c(
      "N schools",
      "N (schools X colleges)",
      "Parameters (incl. FE)",
      "Adj. R²",
      "Within R²"
    ),
    coef_base = c(
      samp_base$n_schools,
      samp_base$n_pairs,
      fit_base$n_parameters,
      fit_base$adj_r2,
      fit_base$within_r2
    ),
    se_base = NA_real_,
    p_base = NA_real_,
    coef_int = c(
      samp_int$n_schools,
      samp_int$n_pairs,
      fit_int$n_parameters,
      fit_int$adj_r2,
      fit_int$within_r2
    ),
    se_int = NA_real_,
    p_int = NA_real_,
    block = "fit"
  )
  
  dplyr::bind_rows(
    tab_coef,
    tibble::tibble(
      term_label = "",
      coef_base = NA_real_,
      se_base = NA_real_,
      p_base = NA_real_,
      coef_int = NA_real_,
      se_int = NA_real_,
      p_int = NA_real_,
      block = "spacer"
    ),
    fit_df
  )
}


make_rq3_kable <- function(tab_df, digits = 3) {
  
  int_rows <- c(
    "N schools",
    "N (schools X colleges)",
    "Parameters (incl. FE)"
  )
  
  dec_rows <- c(
    "Adj. R²",
    "Within R²"
  )
  
  # layout knobs
  label_col_em   <- 35
  num_col_em     <- 4
  table_width_em <- label_col_em + 4*num_col_em + 2
  
  star_code <- function(p) {
    dplyr::case_when(
      is.na(p)  ~ "",
      p < 0.001 ~ "***",
      p < 0.01  ~ "**",
      p < 0.05  ~ "*",
      TRUE      ~ ""
    )
  }
  
  fmt_coef <- function(x, p, row_lab, block) {
    dplyr::case_when(
      block == "header" ~ "",
      row_lab %in% int_rows ~ format(round(x, 0), big.mark = ",", scientific = FALSE, trim = TRUE),
      row_lab %in% dec_rows ~ sprintf(paste0("%.", digits, "f"), x),
      is.na(x) ~ "",
      TRUE ~ paste0(sprintf(paste0("%.", digits, "f"), x), star_code(p))
    )
  }
  
  fmt_se <- function(x, row_lab, block) {
    dplyr::case_when(
      block == "header" ~ "",
      row_lab %in% int_rows ~ "",
      row_lab %in% dec_rows ~ "",
      is.na(x) ~ "",
      TRUE ~ sprintf(paste0("%.", digits, "f"), x)
    )
  }
  
  tab_fmt <- tab_df %>%
    dplyr::mutate(
      coef_base = fmt_coef(coef_base, p_base, term_label, block),
      se_base   = fmt_se(se_base, term_label, block),
      coef_int  = fmt_coef(coef_int, p_int, term_label, block),
      se_int    = fmt_se(se_int, term_label, block)
    )
  
  header_rows <- which(tab_fmt$block == "header")
  fit_start   <- which(tab_fmt$term_label == "N schools")
  
  tbl <- tab_fmt %>%
    dplyr::select(term_label, coef_base, se_base, coef_int, se_int) %>%
    kableExtra::kbl(
      format = "html",
      escape = FALSE,
      align = c("l", "r", "r", "r", "r"),
      col.names = c(" ", "Coef.", "SE", "Coef.", "SE"),
      table.attr = paste0(
        "class='rq3-kable' style='",
        "margin-left:0; margin-right:auto; ",
        "line-height:0.95; ",
        "table-layout:fixed; ",
        "width:", table_width_em, "em;'"
      )
    ) %>%
    kableExtra::add_header_above(c(" " = 1, "Baseline" = 2, "Interaction" = 2)) %>%
    kableExtra::kable_styling(
      font_size = 16,
      full_width = FALSE,
      bootstrap_options = c("condensed"),
      position = "left"
    ) %>%
    kableExtra::column_spec(
      1,
      extra_css = "padding-top: 2px; padding-bottom: 2px; padding-left: 1px; padding-right: 6px;"
    ) %>%
    kableExtra::column_spec(
      2:5,
      extra_css = "padding-top: 2px; padding-bottom: 2px; padding-left: 2px; padding-right: 0px;"
    )
  
  if (length(header_rows) > 0) {
    for (r in header_rows) {
      tbl <- tbl %>%
        kableExtra::row_spec(
          r,
          bold = TRUE,
          extra_css = "padding-top: 2px; padding-bottom: 2px;"
        )
    }
  }
  
  if (length(fit_start) == 1) {
    tbl <- tbl %>%
      kableExtra::row_spec(
        fit_start,
        extra_css = "border-top: 1px solid black;"
      )
  }
  
  tbl_html <- as.character(tbl)
  
  tbl_html <- gsub(
    "width: auto !important;",
    paste0("width: ", table_width_em, "em !important;"),
    tbl_html,
    fixed = TRUE
  )
  
  tbl_html <- gsub(
    "width:auto;",
    paste0("width:", table_width_em, "em;"),
    tbl_html,
    fixed = TRUE
  )
  
  colgroup_html <- paste0(
    "<colgroup>",
    "<col style='width:", label_col_em, "em;'>",
    "<col style='width:", num_col_em, "em;'>",
    "<col style='width:", num_col_em, "em;'>",
    "<col style='width:", num_col_em, "em;'>",
    "<col style='width:", num_col_em, "em;'>",
    "</colgroup>"
  )
  
  tbl_html <- sub(
    "(<table[^>]*>)",
    paste0("\\1", colgroup_html),
    tbl_html
  )
  
  css_fix <- paste0(
    "<style>",
    ".rq3-kable { border-collapse: collapse !important; table-layout: fixed !important; width: ", table_width_em, "em !important; }",
    ".rq3-kable th, .rq3-kable td { padding-left: 1px !important; padding-right: 1px !important; }",
    ".rq3-kable col:nth-child(1) { width: ", label_col_em, "em !important; }",
    ".rq3-kable col:nth-child(2), .rq3-kable col:nth-child(3), .rq3-kable col:nth-child(4), .rq3-kable col:nth-child(5) { width: ", num_col_em, "em !important; }",
    ".rq3-kable thead tr:first-child th:first-child, .rq3-kable thead tr:nth-child(2) th:first-child, .rq3-kable tbody td:first-child { white-space: normal !important; overflow-wrap: normal !important; word-break: normal !important; box-sizing: border-box !important; }",
    ".rq3-kable thead tr:nth-child(2) th:nth-child(2), .rq3-kable thead tr:nth-child(2) th:nth-child(3), .rq3-kable thead tr:nth-child(2) th:nth-child(4), .rq3-kable thead tr:nth-child(2) th:nth-child(5), .rq3-kable tbody td:nth-child(2), .rq3-kable tbody td:nth-child(3), .rq3-kable tbody td:nth-child(4), .rq3-kable tbody td:nth-child(5) { white-space: nowrap !important; box-sizing: border-box !important; }",
    ".rq3-kable tbody tr:last-child td { border-bottom: 1px solid black !important; }",
    ".rq3-kable tbody tr:nth-child(", fit_start, ") td:first-child { border-top: none !important; }",
    "</style>"
  )
  
  htmltools::tagList(
    htmltools::HTML(css_fix),
    htmltools::HTML(tbl_html)
  )
}

rq3_results <- readRDS("results/rq3_pub_interaction_models_master.RDS")
#rq3_results_pubu_allhs <- readRDS("results/rq3_pubuniv_allhs_interaction_models.RDS")
#rq3_results_pubu_instate <- readRDS("results/rq3_pubuniv_instate_interaction_models.RDS")
#rq3_results_pubu_oos <- readRDS("results/rq3_pubuniv_oos_interaction_models.RDS")
```


### __RQ3: Which schools benefit from popular Geomarkets__ {.hide}
#### __Interacting Geomarket popularity (visits per school) and school pct free/reduced lunch__

```{r rq3-test, echo=FALSE, message=FALSE, warning=FALSE}

tab_df <- make_rq3_table_df(
  model_bundle = rq3_results$models$per_sch_frl,
  decile_cutoffs = rq3_results$run_meta$decile_cutoffs
)

tbl <- make_rq3_kable(tab_df)

htmltools::tagList(
  tbl,
  htmltools::HTML(
    "<div style='font-size:0.55em; margin-top:6px; margin-left:0; text-align:left;'>Standard errors two-way clustered by HS state and college. Significance levels: * p &lt; 0.05, ** p &lt; 0.01, *** p &lt; 0.001.</div>"
  )
)
```


### __RQ3: Which schools benefit from popular Geomarkets__ {.hide}
#### __Interacting Geomarket popularity (visits per school) and school pct Black/Hisp/Native__

```{r rq3-blhispnat-per-sch, echo=FALSE, message=FALSE, warning=FALSE}

tab_df <- make_rq3_table_df(
  model_bundle = rq3_results$models$per_sch_blhispnat,
  decile_cutoffs = rq3_results$run_meta$decile_cutoffs
)

tbl <- make_rq3_kable(tab_df)

htmltools::tagList(
  tbl,
  htmltools::HTML(
    "<div style='font-size:0.55em; margin-top:6px; margin-left:0; text-align:left;'>Standard errors two-way clustered by HS state and college. Significance levels: * p &lt; 0.05, ** p &lt; 0.01, *** p &lt; 0.001.</div>"
  )
)
```




### __RQ3: Which schools benefit from popular Geomarkets__ {.hide}
#### Interacting Geomarket popularity with school/neighborhood composition

```{r fig-rq3-me-2x3, echo=FALSE, results="asis", message=FALSE, warning=FALSE}
png_out <- "results/rq3_marginal_effects_2x3.png"

cat(sprintf(
  '<p style="text-align:center; margin:-0.3em 0 0 0; padding:0;"><img src="%s" style="height:75vh; width:auto; max-height:none; max-width:none; cursor:zoom-in;"></p>',
  png_out
))
```

### __RQ3: Visits by public universities to public schools__ {.hide}
#### __Geomarket popularity (visits per school) and school/neighborhood composition__

```{r fig-rq3-pubu-me-3x3-per-sch, echo=FALSE, results="asis", message=FALSE, warning=FALSE}
png_out <- "results/rq3_pubu_marginal_effects_3x3_per_sch.png"

cat(sprintf(
  '<p style="text-align:center; margin:-0.2em 0 0 0; padding:0;"><img src="%s" style="height:70vh; width:auto; max-height:none; max-width:none; cursor:zoom-in;"></p>',
  png_out
))
```

# Discussion

### __Discussion__ {.hide}
#### __Contribution to scholarship__

Main findings

- Consistent with Market Segment Model, most colleges followed regional/national recruiting strategy that prioritized affluent Geomarkets
- Geomarkets explain which high schools receive visits
- Affluent, moderately diverse schools benefit from being in a popular Geomarket

Contribution

- Dominant explanations of selective college access focuses on admissions
  - college-level [@RN3544]; macro forces (e.g., rankings, affirmative action)
- The literature needs to move upstream in two ways, simultaneously
    1. In **process**, from admissions to recruiting [@RN3519]
    1. In **actors**, from colleges to third-party market devices that colleges rely on
- Third-party recruiting infrastructure is a mechanism of **social closure**
  - Shapes which students get pursued by which colleges
  - Can be usefully studied as a set of interacting market devices

### __Discussion__ {.hide}
#### __Infrastructural collusion__

In @RN4982 chap. 4, "The Company We Keep: Colleges and Their Competition," two colleges are competitors if many students send scores to both:

<div style="font-size: 0.91em; line-height: 1.2;">

> In the words of one veteran dean of admissions, 'We are known primarily by the company we keep.' For this reason, colleges actually encourage competition with one another to define their places in the market. Members of the Consortium on Financing Higher Education (COFHE) -- a group of thirty highly selective, high-cost, private institutions -- regularly exchange detailed and confidential information about their applicant pools, admissions strategies, and financial aid programs. 
>
> What every envious admissions officer outside the Consortium understands is that these thirty institutions are strengthened by their collectivity, though they are in fact one another’s principal competitors. The dean of admissions at one COFHE institution knows that there is greater chance of interesting a student who is already considering another COFHE institution than a student who is not. 
>
> The same kind of forces help define collective markets for state colleges, women's colleges, Catholic or other church-related institutions, and even diverse urban institutions that share intense athletic rivalries (p. 45).

</div>

### __Discussion__ {.hide}
#### __Infrastructural collusion__

Peer-patronage logic: @RN4982 observe that college reputation is joint function of (a) social class of students and (b) peer colleges it associates with

- Socioeconomic homophily: like colleges compete for students of like social class

Building this logic into devices, we go from observing to structuring competition

- College Board Enrollment Planning Service (EPS) "competitive analysis" tab: 
  - Locate Geomarkets that showed strong demand for peer colleges
- By relying on shared infrastructure, competitors coordinate without collusion
  
Engineering homophily in social networks [@RN4975]:

- Identify variables that predict connections
- Write algorithms that makes it easy/default to create those connections
- College-facing recruiting products have "interest in my peers" type filter
- Consulting firms (EAB), member assoc (College Board) serve 100s of colleges
  - Incentive to match "right" students to each college [@RN4810]


### __Discussion__ {.hide}
#### __Future research on market devices in edtech__

Three ideal-type origins of third-party market devices in enrollment management

1. Built by (elite) higher ed insiders, based on logic of higher ed insiders
    - Example: Geomarkets, Market Segment Model, and EPS
1. Higher ed insiders apply commercial marketing logic/techniques
    - Example: @RN4565 Geodemographic Segment Analysis Service
1. Private equity firms applying software-as-service models to higher ed
    - Example: EAB's Enroll360 [@koppenheffer_2021]

Future research

- A critical historical sociology of enrollment management industry
  - Logical starting point is the College Board
- Sociology (and economics) has not made edtech an object of inquiry
  - Useful concepts: infrastructures [@star1999ethnography]; market devices
  - "Platform studies in education" literature: how digital platforms organize core functions of schooling [@RN4843]

# Appendix

## RQ0

```{r rq0, echo=FALSE, results = 'asis', eval = TRUE}

library(tidyverse)
library(kableExtra)  # For collapse_rows() to work
library(tools)

knitr::opts_chunk$set(echo = FALSE, message = FALSE)

# Example directories
graphs_dir <- file.path(".", "results", "graphs")

# Revised insert_figure() that does NOT print a heading
insert_figure <- function(rq, metro, graph_type, base_path = graphs_dir) {

  # We'll use 'metro' directly since it's already underscored (e.g. "los_angeles").
  fig_file  <- file.path(base_path, rq, stringr::str_c(rq, "_", metro, "_", graph_type, ".png"))
  cap_file  <- file.path(base_path, rq, stringr::str_c(rq, "_", metro, "_", graph_type, "_title.txt"))
  note_file <- file.path(base_path, rq, stringr::str_c(rq, "_", metro, "_", graph_type, "_note.txt"))

  # Print a heading/caption
  caption_text <- if (file.exists(cap_file)) readLines(cap_file, warn = FALSE) else "No Title Found"
  writeLines(paste0("#### ", caption_text))

  # Output the figure (if it exists)
  if (file.exists(fig_file)) {
    writeLines(stringr::str_c("![](", fig_file, ")\n"))
  } else {
    writeLines(paste("*(Missing figure file:", fig_file, ")*\n"))
  }

  # Print footnotes (if the .txt file exists)
  if (file.exists(note_file)) {
    notes_txt <- readLines(note_file, warn = FALSE)
    writeLines(
      stringr::str_c(
        "<div class=\"footnote\">",
        paste0(notes_txt, collapse = "</br>"),
        "</div>"
      )
    )
  }
}

# 2) Define your *underscored* metros / RQs
  metros <- c("atlanta","bay_area","boston","chicago","cleveland","dallas","dc_maryland_virginia","detroit","houston","long_island","los_angeles","miami","new_york_city","northern_new_jersey","orange_county","philadelphia","san_diego")

  # metros <- c("bay_area","chicago")
  
rqs <- c("rq1")

# For RQ1, we have two plot types: "race" and "ses"
graph_types_rq1 <- c("race", "ses")

# 3) Main loop
for (m in metros) {

  # Convert underscores -> spaces, then title case
  # But add special checks for "dc_maryland_virginia" and "bay_area"
  if (m == "dc_maryland_virginia") {
    heading_text <- "D.C., Maryland, and Virginia"
  } else if (m == "bay_area") {
    heading_text <- "Bay Area"
  } else {
    # Normal logic
    heading_text <- gsub("_", " ", m, fixed = TRUE)
    heading_text <- tools::toTitleCase(heading_text)
    heading_text <- str_c(heading_text, " area")
  }

  # Print a top-level heading for the metro area
  # writeLines(str_c("### ", heading_text))

  for (rq in rqs) {
    
    # For clarity, parse out the numeric portion
    rq_num <- str_replace(string = rq, pattern = "rq", replacement = "")

    # ---------------------------------------------
    # RQ1 Logic
    # ---------------------------------------------
    if (rq == "rq1") {

      # Each metro has 2 plots: "race" & "ses"
      for (i in seq_along(graph_types_rq1)) {
        g <- graph_types_rq1[i]

        # The first RQ1 slide for a metro has a real heading;
        # subsequent ones are hidden slides (".hide")
        writeLines(str_c("### ", heading_text, if_else(i == 1, "", " {.hide}")))
        insert_figure(rq = rq, metro = m, graph_type = g)
      }
      
      # Add RQ1 map
      writeLines(str_c("### ", heading_text, " {.hide}"))
      writeLines(str_c("#### High school recruiting visits pattern"))
      
      rq1_map <- str_c('./results/maps/rq0_map_', m, '.html')
      
      writeLines(str_c('<iframe data-src="', rq1_map, '" src="about:blank" width=100% height=100% allowtransparency="true"></iframe>'))
      writeLines('<p class="fullscreen">Fullscreen</p>')
      writeLines('<div class="footnote">Figure Notes:<br>- Race/ethnicity categories not available in 1980 Census: Asian, non-Hispanic; Two+ races, non-Hispanic; NHPI, non-Hispanic; AIAN non-Hispanic<br>- Household income measured using 2024 CPI</div>')
    }

  } # end rq loop

} # end metros loop
```

## RQ1

### __RQ1: Do visit patterns match Market Segment Model?__ {.hide}
#### __Freshman enrollment by residency status (in-state, out-of-state, international)__

```{r fig-enrollment-bar-residency-appendix, echo=FALSE, results="asis", message=FALSE, warning=FALSE}
pdf_in  <- "results/enrollment_bar_residency_appendix.pdf"
png_out <- "results/enrollment_bar_residency_appendix.png"
if (!file.exists(png_out) || file.info(png_out)$mtime < file.info(pdf_in)$mtime) {
  if (!requireNamespace("magick", quietly = TRUE)) {
    stop("Install 'magick': install.packages('magick')")
  }
  img <- magick::image_read_pdf(pdf_in, density = 220)
  magick::image_write(img, path = png_out, format = "png")
}
cat(sprintf(
  '<p style="text-align:center;"><img src="%s" style="width:70%%; height:auto; cursor:zoom-in;"></p>',
  png_out
))
```

### __RQ1: Do visit patterns match Market Segment Model?__ {.hide}
#### __Top 30 Geomarkets ranked by visits per 1,000 12th graders (public schools)__
```{r top30-pub-g12k, echo=FALSE, message=FALSE, warning=FALSE}
readRDS("results/top30_geo_pub_g12k.RDS") %>%
  kableExtra::kbl(
    digits  = 1,
    caption = NULL,
    align   = c("r", "l", rep("r", ncol(.) - 2))
  ) %>%
  kableExtra::kable_classic(full_width = TRUE) %>%
  kableExtra::kable_styling(font_size = 16) %>%
  kableExtra::scroll_box(height = "82vh", width = "100%")
```

### __RQ1: Do visit patterns match Market Segment Model?__ {.hide}
#### __Top 30 Geomarkets ranked by visits 1,000 12th graders (private schools)__
```{r top30-priv-g12k, echo=FALSE, message=FALSE, warning=FALSE}
readRDS("results/top30_geo_priv_g12k.RDS") %>%
  kableExtra::kbl(
    digits  = 1,
    caption = NULL,
    align   = c("r", "l", rep("r", ncol(.) - 2))
  ) %>%
  kableExtra::kable_classic(full_width = TRUE) %>%
  kableExtra::kable_styling(font_size = 16) %>%
  kableExtra::scroll_box(height = "82vh", width = "100%")
```

### __RQ1: Do visit patterns match Market Segment Model?__ {.hide}
#### __Top 30 Geomarkets ranked by visits per 1,000 12th graders (public schools, national market)__
```{r top30-pub-g12k-national, echo=FALSE, message=FALSE, warning=FALSE}
readRDS("results/top30_geo_pub_national_g12k.RDS") %>%
  kableExtra::kbl(
    digits  = 1,
    caption = NULL,
    align   = c("r", "l", rep("r", ncol(.) - 2))
  ) %>%
  kableExtra::kable_classic(full_width = TRUE) %>%
  kableExtra::kable_styling(font_size = 16) %>%
  kableExtra::scroll_box(height = "82vh", width = "100%")
```

### __RQ1: Do visit patterns match Market Segment Model?__ {.hide}
#### __Top 30 Geomarkets ranked by visits 1,000 12th graders (private schools, national market)__
```{r top30-priv-g12k-national, echo=FALSE, message=FALSE, warning=FALSE}
readRDS("results/top30_geo_priv_national_g12k.RDS") %>%
  kableExtra::kbl(
    digits  = 1,
    caption = NULL,
    align   = c("r", "l", rep("r", ncol(.) - 2))
  ) %>%
  kableExtra::kable_classic(full_width = TRUE) %>%
  kableExtra::kable_styling(font_size = 16) %>%
  kableExtra::scroll_box(height = "82vh", width = "100%")
```


## RQ2

### RQ2: Do Geomarkets explain which schools get visits? {.hide}
#### Model fit, fixed effects LPMs of visits to private schools near Geomarket boundaries

<div style="height: 14px;"></div>

```{r rq2-model-fit-priv-border-compare, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

tab_df_priv_bc_nocov <- readRDS("results/tab_priv_border_compare_nocov_ij.RDS")
tab_df_priv_bc_cov   <- readRDS("results/tab_priv_border_compare_cov_ij.RDS")

make_rq2_border_fit_table(tab_df_priv_bc_nocov, tab_df_priv_bc_cov)

```

### __RQ2: Do Geomarkets explain which schools get visits?__ {.hide}
#### __Model fit statistics, Fixed effects LPMs of visits to public schools by public univs__

<div style="height: 14px;"></div>

```{r rq2-model-fit-pubu-pubhs-all, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

tab_pubu_pubhs_all_ij <- readRDS("results/tab_pubu_pubhs_all_ij.RDS")
make_rq2_fit_table(tab_pubu_pubhs_all_ij)

```

### __RQ2: Do Geomarkets explain which schools get visits?__ {.hide}
#### __Model fit statistics, Fixed effects LPMs of in-state visits to public schools by public univs__

<div style="height: 14px;"></div>

```{r rq2-model-fit-pubu-pubhs-instate, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

tab_pubu_pubhs_instate_ij <- readRDS("results/tab_pubu_pubhs_instate_ij.RDS")
make_rq2_fit_table(tab_pubu_pubhs_instate_ij)

```

### __RQ2: Do Geomarkets explain which schools get visits?__ {.hide}
#### __Model fit statistics, Fixed effects LPMs of out-of-state visits to public schools by public univs__

<div style="height: 14px;"></div>

```{r rq2-model-fit-pubu-pubhs-oos, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

tab_pubu_pubhs_oos_ij <- readRDS("results/tab_pubu_pubhs_oos_ij.RDS")
make_rq2_fit_table(tab_pubu_pubhs_oos_ij)

```

### __RQ2: Do Geomarkets explain which schools get visits?__ {.hide}
#### _Percent free-reduced lunch: HS close to border are more similar to each other in 70.6% of borders_

![](./results/comparison_lunch.png)

### __RQ2: Do Geomarkets explain which schools get visits?__ {.hide}
#### _Median household income: HS close to border are more similar to each other in 82.1% of borders_

![](./results/comparison_income.png)

## RQ3

### __RQ3: Which schools benefit from popular Geomarkets__ {.hide}
#### __Interacting Geomarket popularity (visits per 1k 12th graders) and school pct free/reduced lunch__

```{r rq3-frl-per-g12k, echo=FALSE, message=FALSE, warning=FALSE}

tab_df <- make_rq3_table_df(
  model_bundle = rq3_results$models$per_g12k_frl,
  decile_cutoffs = rq3_results$run_meta$decile_cutoffs
)

tbl <- make_rq3_kable(tab_df)

htmltools::tagList(
  tbl,
  htmltools::HTML(
    "<div style='font-size:0.55em; margin-top:6px; margin-left:0; text-align:left;'>Standard errors two-way clustered by HS state and college. Significance levels: * p &lt; 0.05, ** p &lt; 0.01, *** p &lt; 0.001.</div>"
  )
)
```

### __RQ3: Which schools benefit from popular Geomarkets__ {.hide}
#### __Interacting Geomarket popularity (visits per school) and ZIP-code mean household income__

```{r rq3-zipinc-per-sch, echo=FALSE, message=FALSE, warning=FALSE}

tab_df <- make_rq3_table_df(
  model_bundle = rq3_results$models$per_sch_zipinc,
  decile_cutoffs = rq3_results$run_meta$decile_cutoffs
)

tbl <- make_rq3_kable(tab_df)

htmltools::tagList(
  tbl,
  htmltools::HTML(
    "<div style='font-size:0.55em; margin-top:6px; margin-left:0; text-align:left;'>Standard errors two-way clustered by HS state and college. Significance levels: * p &lt; 0.05, ** p &lt; 0.01, *** p &lt; 0.001.</div>"
  )
)
```

### __RQ3: Which schools benefit from popular Geomarkets__ {.hide}
#### __Interacting Geomarket popularity (visits per 1k 12th graders) and ZIP-code mean household inc__

```{r rq3-zipinc-per-g12k, echo=FALSE, message=FALSE, warning=FALSE}

tab_df <- make_rq3_table_df(
  model_bundle = rq3_results$models$per_g12k_zipinc,
  decile_cutoffs = rq3_results$run_meta$decile_cutoffs
)

tbl <- make_rq3_kable(tab_df)

htmltools::tagList(
  tbl,
  htmltools::HTML(
    "<div style='font-size:0.55em; margin-top:6px; margin-left:0; text-align:left;'>Standard errors two-way clustered by HS state and college. Significance levels: * p &lt; 0.05, ** p &lt; 0.01, *** p &lt; 0.001.</div>"
  )
)
```

### __RQ3: Which schools benefit from popular Geomarkets__ {.hide}
#### __Interacting Geomarket popularity (visits per 1k 12th graders) and school pct Black/Hisp/Native__

```{r rq3-blhispnat-per-g12k, echo=FALSE, message=FALSE, warning=FALSE}


tab_df <- make_rq3_table_df(
  model_bundle = rq3_results$models$per_g12k_blhispnat,
  decile_cutoffs = rq3_results$run_meta$decile_cutoffs
)

tbl <- make_rq3_kable(tab_df)

htmltools::tagList(
  tbl,
  htmltools::HTML(
    "<div style='font-size:0.55em; margin-top:6px; margin-left:0; text-align:left;'>Standard errors two-way clustered by HS state and college. Significance levels: * p &lt; 0.05, ** p &lt; 0.01, *** p &lt; 0.001.</div>"
  )
)
```


### __RQ3: Which schools benefit from popular Geomarkets__ {.hide}
#### Interacting Geomarket popularity with school composition

```{r fig-rq3-me-2x2, echo=FALSE, results="asis", message=FALSE, warning=FALSE}
png_out <- "results/rq3_marginal_effects_2x2.png"

cat(sprintf(
  '<p style="text-align:center;"><img src="%s" style="width:100%%; height:auto; cursor:zoom-in;"></p>',
  png_out
))
```

### __RQ3 appendix: Public univ visits to all schools__ {.hide}
#### __Interacting Geomarket popularity (visits per school) and school pct free/reduced lunch__

```{r rq3-pubu-allhs-frl-per-sch, echo=FALSE, message=FALSE, warning=FALSE}

rq3_results_this <- readRDS("results/rq3_pubuniv_allhs_interaction_models.RDS")

tab_df <- make_rq3_table_df(
  model_bundle = rq3_results_this$models$per_sch_frl,
  decile_cutoffs = rq3_results_this$run_meta$decile_cutoffs
)

tbl <- make_rq3_kable(tab_df)

rm(rq3_results_this)
invisible(gc())

htmltools::tagList(
  tbl,
  htmltools::HTML(
    "<div style='font-size:0.55em; margin-top:6px; margin-left:0; text-align:left;'>Public universities only; all public high schools. Standard errors two-way clustered by HS state and college. Significance levels: * p &lt; 0.05, ** p &lt; 0.01, *** p &lt; 0.001.</div>"
  )
)
```

### __RQ3 appendix: Public univ visits to in-state schools__ {.hide}
#### __Interacting Geomarket popularity (visits per school) and school pct free/reduced lunch__

```{r rq3-pubu-instate-frl-per-sch, echo=FALSE, message=FALSE, warning=FALSE}

rq3_results_this <- readRDS("results/rq3_pubuniv_instate_interaction_models.RDS")

tab_df <- make_rq3_table_df(
  model_bundle = rq3_results_this$models$per_sch_frl,
  decile_cutoffs = rq3_results_this$run_meta$decile_cutoffs
)

tbl <- make_rq3_kable(tab_df)

rm(rq3_results_this)
invisible(gc())

htmltools::tagList(
  tbl,
  htmltools::HTML(
    "<div style='font-size:0.55em; margin-top:6px; margin-left:0; text-align:left;'>Public universities only; in-state public high schools. Standard errors two-way clustered by HS state and college. Significance levels: * p &lt; 0.05, ** p &lt; 0.01, *** p &lt; 0.001.</div>"
  )
)
```

### __RQ3 appendix: Public univ visits to out-of-state schools__ {.hide}
#### __Interacting Geomarket popularity (visits per school) and school pct free/reduced lunch__

```{r rq3-pubu-oos-frl-per-sch, echo=FALSE, message=FALSE, warning=FALSE}
rm(list = setdiff(ls(), c("make_rq3_table_df", "make_rq3_kable")))
invisible(gc())
rq3_results_this <- readRDS("results/rq3_pubuniv_oos_interaction_models.RDS")

tab_df <- make_rq3_table_df(
  model_bundle = rq3_results_this$models$per_sch_frl,
  decile_cutoffs = rq3_results_this$run_meta$decile_cutoffs
)

tbl <- make_rq3_kable(tab_df)

rm(rq3_results_this)
invisible(gc())

htmltools::tagList(
  tbl,
  htmltools::HTML(
    "<div style='font-size:0.55em; margin-top:6px; margin-left:0; text-align:left;'>Public universities only; out-of-state public high schools. Standard errors two-way clustered by HS state and college. Significance levels: * p &lt; 0.05, ** p &lt; 0.01, *** p &lt; 0.001.</div>"
  )
)
```


# References

::: {#refs}
:::
